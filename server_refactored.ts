...
async function getAuthUser(req: express.Request) {
  ...
  const profile = await db.find('profiles', (p) => p.id === token || p.email === token || p.auth_user_id === token)[0];
  if (!profile) return null;

  const student = profile.role === 'STUDENT' ? await db.find('students', (s) => s.profile_id === profile.id)[0] : null;
  const teacher = profile.role === 'TEACHER' ? await db.find('teachers', (t) => t.profile_id === profile.id)[0] : null;
  const admin = profile.role === 'ADMIN' ? await db.find('administrators', (a) => a.profile_id === profile.id)[0] : null;
...
app.post('/api/auth/register', (req, res) => {
  ...
  if (role === 'STUDENT') {
    ...
    const existingStudent = await db.find('students', (s) => s.roll_number && s.roll_number.toLowerCase() === cleanRoll.toLowerCase())[0];
    if (existingStudent) {
  ...
  if (role === 'TEACHER') {
    ...
    const existingTeacher = await db.find('teachers', (t) => t.employee_code && t.employee_code.toLowerCase() === cleanCode.toLowerCase())[0];
    if (existingTeacher) {
  ...
  const existing = await db.find('profiles', (p) => p.email.toLowerCase() === email.toLowerCase())[0];
  if (existing) {
  ...
  await db.logAudit('USER_REGISTERED', 'profile', profile.id, profile.email, profile.id, { role });
  await db.createNotification(profile.id, 'Welcome to SC EduSense AI', 'Your academic account has been initialized. Explore courses and academic intelligence.', 'INFO', '/');
...
app.post('/api/auth/forgot-password', (req, res) => {
  ...
  const profile = await db.find('profiles', (p) => p.email.toLowerCase() === email.toLowerCase())[0];
  if (!profile) {
  ...
  await db.logAudit('PASSWORD_RESET_REQUESTED', 'profile', profile.id, profile.email, profile.id);
...
app.post('/api/auth/reset-password', (req, res) => {
  ...
  await db.logAudit('PASSWORD_RESET_COMPLETED', 'profile', profile.id, profile.email, profile.id);
...
app.post('/api/auth/login', (req, res) => {
  ...
  await db.logAudit('USER_LOGIN', 'profile', profile.id, profile.email, profile.id);
...
app.post('/api/auth/quick-switch', (req, res) => {
  const { role } = req.body;
  const profile = await db.find('profiles', (p) => p.role === role)[0];
  if (!profile) {
...
app.get('/api/departments', (req, res) => {
  let departments = await db.find('departments');
...
app.post('/api/departments', (req, res) => {
  ...
  const existing = await db.find('departments', (d) => d.name.toLowerCase() === name.toLowerCase() || d.shortCode.toLowerCase() === shortCode.toLowerCase())[0];
  if (existing) {
  ...
  await db.logAudit('DEPARTMENT_CREATED', 'department', department.id, auth.profile.email, auth.profile.id, { name, shortCode });
...
app.get('/api/courses', (req, res) => {
  ...
  const auth = await getAuthUser(req);
  let courses = await db.find('courses');
  ...
  if (faculty_id && faculty_id !== 'All' && String(faculty_id).trim() !== '') {
    ...
    const allClasses = await db.find('classes');
    
    allClasses.forEach((cls) => {
      if (cls.teacher_id === facQuery) {
        matchedClassCourseIds.add(cls.course_id);
      } else {
        const t = await db.findById('teachers', cls.teacher_id);
        const p = t ? await db.findById('profiles', t.profile_id) : null;
  ...
  const enriched = courses.map((course) => {
    const classes = await db.find('classes', (cl) => cl.course_id === course.id);
    const enrollments = await db.find('enrollments', (en) => en.course_id === course.id && (en.status === 'ENROLLED' || !en.status));
    ...
    if (!course.is_nptel && classes.length > 0) {
      const firstTeacher = await db.findById('teachers', classes[0].teacher_id);
      const firstProf = firstTeacher ? await db.findById('profiles', firstTeacher.profile_id) : null;
      if (firstProf) {
    ...
    if (auth?.student) {
      const userEnr = await db.find('enrollments', (en) => en.course_id === course.id && en.student_id === auth.student!.id && en.status !== 'DROPPED')[0];
      if (userEnr) {
        isEnrolled = true;
...
app.get('/api/courses/:id', (req, res) => {
  ...
  const classes = await db.find('classes', (c) => c.course_id === course.id).map((cls) => {
    const teacher = await db.findById('teachers', cls.teacher_id);
    const teacherProfile = teacher ? await db.findById('profiles', teacher.profile_id) : null;
    const studentsCount = await db.find('class_students', (cs) => cs.class_id === cls.id).length;
    return {
      ...cls,
  ...
  const assignments = await db.find('assignments', (a) => a.course_id === course.id);
  const examinations = await db.find('examinations', (e) => e.course_id === course.id);
  const enrollments = await db.find('enrollments', (en) => en.course_id === course.id && (en.status === 'ENROLLED' || !en.status));
  ...
  if (auth?.student) {
    ...
    if (studentEnrollment) {
      ...
      const courseSessions = await db.find('attendance_sessions', (as) => courseClassIds.includes(as.class_id));
      const sessionIds = courseSessions.map((s) => s.id);
      const records = await db.find('attendance_records', (ar) => ar.student_id === auth.student.id && sessionIds.includes(ar.session_id));
      if (records.length > 0) {
...
app.post('/api/courses', (req, res) => {
  ...
  const existing = await db.find('courses', (c) => c.code.toUpperCase() === cleanCode)[0];
  if (existing) {
  ...
  await db.logAudit('COURSE_CREATED', 'course', course.id, auth.profile.email, auth.profile.id, { 
    code: course.code, 
    name: course.name, 
...
app.put('/api/courses/:id', (req, res) => {
  ...
  const existing = await db.findById('courses', req.params.id);
  if (!existing) {
  ...
  if (code && code.toUpperCase() !== existing.code.toUpperCase()) {
    const duplicate = await db.find('courses', (c) => c.code.toUpperCase() === code.toUpperCase() && c.id !== existing.id)[0];
    if (duplicate) {
  ...
  const updated: any = await db.update('courses', req.params.id, {
  ...
  await db.logAudit('COURSE_UPDATED', 'course', updated.id, auth.profile.email, auth.profile.id, { code: updated.code, name: updated.name });
...
app.delete('/api/courses/:id', (req, res) => {
  ...
  const enrollments = await db.find('enrollments', (en) => en.course_id === course.id && en.status === 'ENROLLED');
  if (enrollments.length > 0) {
    await db.update('courses', course.id, { status: 'ARCHIVED', updated_at: new Date().toISOString() });
    await db.logAudit('COURSE_ARCHIVED', 'course', course.id, auth.profile.email, auth.profile.id, { reason: 'Has active enrollments' });
  ...
  const classes = await db.find('classes', (cl) => cl.course_id === course.id);
  classes.forEach((c) => await db.delete('classes', c.id));

  await db.delete('courses', course.id);
  await db.logAudit('COURSE_DELETED', 'course', course.id, auth.profile.email, auth.profile.id);
...
app.get('/api/classes', (req, res) => {
  ...
  let classes = await db.find('classes');
  ...
  const enriched = classes.map((cls) => {
    const course = await db.findById('courses', cls.course_id);
    const teacher = await db.findById('teachers', cls.teacher_id);
    const teacherProfile = teacher ? await db.findById('profiles', teacher.profile_id) : null;
    const studentsCount = await db.find('class_students', (cs) => cs.class_id === cls.id).length;
...
app.get('/api/classes/:id', (req, res) => {
  ...
  const classStudents = await db.find('class_students', (cs) => cs.class_id === cls.id);
  const students = classStudents.map((cs) => {
    const student = await db.findById('students', cs.student_id);
    const profile = student ? await db.findById('profiles', student.profile_id) : null;
    return {
      ...cs,
  ...
  const assignments = await db.find('assignments', (a) => a.class_id === cls.id);
  const attendanceSessions = await db.find('attendance_sessions', (as) => as.class_id === cls.id);
...
app.post('/api/classes', (req, res) => {
  ...
  await db.logAudit('CLASS_CREATED', 'class', newClass.id, auth.profile.email, auth.profile.id, { section: section_name });
...
app.post('/api/classes/:id/students', (req, res) => {
  ...
  const targetClass = await db.findById('classes', classId);
  if (!targetClass) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Class not found.' } });
  }

  const existing = await db.find('class_students', (cs) => cs.class_id === classId && cs.student_id === student_id)[0];
  if (existing) {
  ...
  const classStudent = await db.insert('class_students', {
    class_id: classId,
    student_id
  });

  // Also ensure course enrollment
  const courseEnrollment = await db.find('enrollments', (en) => en.course_id === targetClass.course_id && en.student_id === student_id)[0];
  if (!courseEnrollment) {
...
app.delete('/api/classes/:id/students/:studentId', (req, res) => {
  ...
  const record = await db.find('class_students', (cs) => cs.class_id === req.params.id && cs.student_id === req.params.studentId)[0];
  if (record) {
    await db.delete('class_students', record.id);
  }
...
app.post('/api/enrollments', (req, res) => {
  ...
  const course = await db.findById('courses', course_id);
  if (!course) {
  ...
  const existingActive = await db.find('enrollments', (e) => e.course_id === course_id && e.student_id === auth.student!.id && e.status !== 'DROPPED')[0];
  if (existingActive) {
  ...
  // Check seat capacity
  const activeEnrollments = await db.find('enrollments', (en) => en.course_id === course_id && (en.status === 'ENROLLED' || !en.status));
  const maxSeats = Number(course.max_seats) || 60;
  ...
  if (!targetClassId && !course.is_nptel) {
    const defaultClass = await db.find('classes', (c) => c.course_id === course_id)[0];
    if (defaultClass) {
  ...
  if (targetClassId && enrollmentStatus === 'ENROLLED') {
    const existingClassStudent = await db.find('class_students', (cs) => cs.class_id === targetClassId && cs.student_id === auth.student!.id)[0];
...
app.post('/api/enrollments/:id/drop', (req, res) => {
  ...
  const enrollment = await db.findById('enrollments', req.params.id);
  if (!enrollment) {
  ...
  const course = await db.findById('courses', enrollment.course_id);
  const now = new Date().toISOString();

  // Update enrollment status to DROPPED
  const updatedEnrollment = await db.update('enrollments', enrollment.id, {
    status: 'DROPPED',
    dropped_at: now
  });

  // Remove student from any class sections for this course
  const classesForCourse = await db.find('classes', (cls) => cls.course_id === enrollment.course_id);
  const classIds = classesForCourse.map((c) => c.id);
  const classStudentRecords = await db.find('class_students', (cs) => classIds.includes(cs.class_id) && cs.student_id === enrollment.student_id);
  classStudentRecords.forEach((record) => {
    await db.delete('class_students', record.id);
  });

  // Auto-promote first waitlisted student if any
  const waitlisted = await db.find('enrollments', (en) => en.course_id === enrollment.course_id && en.status === 'WAITLISTED');
  if (waitlisted.length > 0) {
    const nextStudentEnrollment = waitlisted[0];
    await db.update('enrollments', nextStudentEnrollment.id, {
      status: 'ENROLLED',
      promoted_at: now
    });

    const nextStudent = await db.findById('students', nextStudentEnrollment.student_id);
    if (nextStudent) {
      await db.createNotification(
  ...
  const studentProfileId = auth.profile.role === 'STUDENT' ? auth.profile.id : (await db.findById('students', enrollment.student_id)?.profile_id || auth.profile.id);
  await db.createNotification(
    studentProfileId,
    'Course Dropped',
...
app.get('/api/enrollments', (req, res) => {
  ...
  let enrollments = await db.find('enrollments');
  ...
  const enriched = enrollments.map((en) => {
    const course = await db.findById('courses', en.course_id);
    const student = await db.findById('students', en.student_id);
    const studentProfile = student ? await db.findById('profiles', student.profile_id) : null;
    ...
    if (course) {
      const classes = await db.find('classes', (c) => c.course_id === course.id);
      const classIds = classes.map((c) => c.id);
      
      // Attendance
      const sessions = await db.find('attendance_sessions', (s) => classIds.includes(s.class_id));
      const sessionIds = sessions.map((s) => s.id);
      if (sessionIds.length > 0) {
        const records = await db.find('attendance_records', (ar) => ar.student_id === en.student_id && sessionIds.includes(ar.session_id));
        if (records.length > 0) {
          const present = records.filter((r) => r.status === 'PRESENT').length;
          const late = records.filter((r) => r.status === 'LATE').length;
      ...
      // Assignments
      const courseAssignments = await db.find('assignments', (a) => a.course_id === course.id);
      const assignmentIds = courseAssignments.map((a) => a.id);
      const submissions = await db.find('assignment_submissions', (as) => as.student_id === en.student_id && assignmentIds.includes(as.assignment_id));
      ...
      // Exam grades
      const courseExams = await db.find('examinations', (e) => e.course_id === course.id);
      const examIds = courseExams.map((e) => e.id);
      const results = await db.find('exam_results', (er) => er.student_id === en.student_id && examIds.includes(er.examination_id));
...
app.get('/api/registrations/audit', (req, res) => {
  ...
  let auditLogs = await db.find('audit_logs', (log) => 
...
app.get('/api/assignments', (req, res) => {
  ...
  let assignments = await db.find('assignments');
  ...
  const enriched = assignments.map((asg) => {
    const course = await db.findById('courses', asg.course_id);
    const classItem = await db.findById('classes', asg.class_id);
    const teacher = await db.findById('teachers', asg.teacher_id);
    const teacherProfile = teacher ? await db.findById('profiles', teacher.profile_id) : null;
    const submissions = await db.find('assignment_submissions', (s) => s.assignment_id === asg.id);
...
app.get('/api/assignments/:id', (req, res) => {
  const auth = await getAuthUser(req);
  const asg = await db.findById('assignments', req.params.id);
  if (!asg) {
  ...
  const submissions = await db.find('assignment_submissions', (s) => s.assignment_id === asg.id).map((sub) => {
    const student = await db.findById('students', sub.student_id);
    const studentProfile = student ? await db.findById('profiles', student.profile_id) : null;
    return {
      ...sub,
...
app.post('/api/assignments', (req, res) => {
  ...
  const teacherId = auth.teacher ? auth.teacher.id : (await db.find('teachers')[0]?.id || 'admin_teacher');
  ...
  const classStudents = await db.find('class_students', (cs) => cs.class_id === class_id);
  classStudents.forEach((cs) => {
    const st = await db.findById('students', cs.student_id);
    if (st) {
      await db.createNotification(
        st.profile_id,
  ...
  await db.logAudit('ASSIGNMENT_CREATED', 'assignment', assignment.id, auth.profile.email, auth.profile.id, { title });
...
app.post('/api/submissions', (req, res) => {
  ...
  const assignment = await db.findById('assignments', assignment_id);
  if (!assignment) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Assignment not found.' } });
  }

  const existing = await db.find('assignment_submissions', (s) => s.assignment_id === assignment_id && s.student_id === auth.student!.id)[0];
  let submission;

  const isLate = new Date() > new Date(assignment.due_date);

  if (existing) {
    submission = await db.update('assignment_submissions', existing.id, {
      submission_text: submission_text || existing.submission_text,
      attachment_url: attachment_url || existing.attachment_url,
      submitted_at: await db.now(),
      status: isLate ? 'LATE' : 'RESUBMITTED'
    });
  } else {
    submission = await db.insert('assignment_submissions', {
      assignment_id,
      student_id: auth.student.id,
      submission_text,
      attachment_url,
  ...
  await db.logAudit('ASSIGNMENT_SUBMITTED', 'assignment_submission', submission?.id, auth.profile.email, auth.profile.id, { assignment_id });
...
app.put('/api/submissions/:id/evaluate', (req, res) => {
  ...
  const submission = await db.findById('assignment_submissions', req.params.id);
  if (!submission) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
  }

  const assignment = await db.findById('assignments', submission.assignment_id);
  ...
  const updated = await db.update('assignment_submissions', submission.id, {
    marks_obtained: Number(marks_obtained),
    feedback: feedback || '',
    evaluated_by: auth.teacher?.id || auth.profile.id,
    evaluated_at: await db.now(),
    status: 'EVALUATED'
  });

  // Notify student
  const student = await db.findById('students', submission.student_id);
  if (student) {
    await db.createNotification(
      student.profile_id,
  ...
  await db.logAudit('SUBMISSION_EVALUATED', 'assignment_submission', submission.id, auth.profile.email, auth.profile.id, { marks_obtained });
...
app.get('/api/attendance/sessions', (req, res) => {
  ...
  let sessions = await db.find('attendance_sessions');
  ...
  const enriched = sessions.map((session) => {
    const classItem = await db.findById('classes', session.class_id);
    const records = await db.find('attendance_records', (r) => r.session_id === session.id);
    return {
      ...session,
      class: classItem,
...
app.get('/api/attendance/sessions/:id', (req, res) => {
  const session = await db.findById('attendance_sessions', req.params.id);
  if (!session) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Attendance session not found.' } });
  }

  const classItem = await db.findById('classes', session.class_id);
  const records = await db.find('attendance_records', (r) => r.session_id === session.id).map((rec) => {
    const student = await db.findById('students', rec.student_id);
    const profile = student ? await db.findById('profiles', student.profile_id) : null;
    return {
      ...rec,
      student: student ? { ...student, profile } : null
...
app.post('/api/attendance/sessions', (req, res) => {
  ...
  let session = await db.find('attendance_sessions', (s) => s.class_id === class_id && s.session_date === session_date)[0];

  if (!session) {
    session = await db.insert('attendance_sessions', {
      class_id,
      teacher_id: auth.teacher?.id || (await db.find('teachers')[0]?.id || 'faculty'),
      session_date,
      session_topic: session_topic || ''
    });
  } else {
    await db.update('attendance_sessions', session.id, {
  ...
  records.forEach((rec: { student_id: string; status: 'PRESENT' | 'ABSENT' | 'LATE' | 'EXCUSED'; remarks?: string }) => {
    const existingRec = await db.find('attendance_records', (r) => r.session_id === session.id && r.student_id === rec.student_id)[0];
    if (existingRec) {
      await db.update('attendance_records', existingRec.id, {
        status: rec.status,
        remarks: rec.remarks
      });
    } else {
  ...
  await db.logAudit('ATTENDANCE_LOGGED', 'attendance_session', session.id, auth.profile.email, auth.profile.id, { class_id, session_date, count: records.length });
...
app.get('/api/examinations', (req, res) => {
  ...
  let exams = await db.find('examinations');
  if (course_id) {
  ...
  const enriched = exams.map((exam) => {
    const course = await db.findById('courses', exam.course_id);
    const classItem = exam.class_id ? await db.findById('classes', exam.class_id) : null;
    const teacher = await db.findById('teachers', exam.teacher_id);
    const teacherProfile = teacher ? await db.findById('profiles', teacher.profile_id) : null;
    const results = await db.find('exam_results', (r) => r.examination_id === exam.id);
...
app.get('/api/examinations/:id', (req, res) => {
  const auth = await getAuthUser(req);
  const exam = await db.findById('examinations', req.params.id);
  if (!exam) {
  ...
  const results = await db.find('exam_results', (r) => r.examination_id === exam.id).map((resRecord) => {
    const student = await db.findById('students', resRecord.student_id);
    const profile = student ? await db.findById('profiles', student.profile_id) : null;
    return {
      ...resRecord,
      student: student ? { ...student, profile } : null
...
app.post('/api/examinations', (req, res) => {
  ...
  const exam = await db.insert('examinations', {
    course_id,
    class_id,
    teacher_id: auth.teacher?.id || (await db.find('teachers')[0]?.id || 'faculty'),
    name,
  ...
  await db.logAudit('EXAM_CREATED', 'examination', exam.id, auth.profile.email, auth.profile.id, { name });
...
app.post('/api/exam-results/bulk', (req, res) => {
  ...
  const exam = await db.findById('examinations', examination_id);
  if (!exam) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Examination not found.' } });
  }
  ...
  results.forEach((item: { student_id: string; marks_obtained: number; grade?: string; remarks?: string }) => {
    ...
    const existing = await db.find('exam_results', (r) => r.examination_id === examination_id && r.student_id === item.student_id)[0];
    if (existing) {
      await db.update('exam_results', existing.id, {
    ...
    // Notify student
    const student = await db.findById('students', item.student_id);
    if (student) {
      await db.createNotification(
        student.profile_id,
  ...
  await db.logAudit('EXAM_RESULTS_RECORDED', 'examination', examination_id, auth.profile.email, auth.profile.id, { count: results.length });
...
app.get('/api/reports/student/:studentId', (req, res) => {
  const student = await db.findById('students', req.params.studentId);
  if (!student) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Student not found.' } });
  }

  const profile = await db.findById('profiles', student.profile_id);
  const analytics = calculateStudentAnalytics(student.id);

  const enrollments = await db.find('enrollments', (e) => e.student_id === student.id).map((en) => {
    const course = await db.findById('courses', en.course_id);
    return { ...en, course };
  });

  const submissions = await db.find('assignment_submissions', (s) => s.student_id === student.id).map((sub) => {
    const assignment = await db.findById('assignments', sub.assignment_id);
    return { ...sub, assignment };
  });

  const examResults = await db.find('exam_results', (r) => r.student_id === student.id).map((er) => {
    const exam = await db.findById('examinations', er.examination_id);
    return { ...er, exam };
  });

  const attendanceRecords = await db.find('attendance_records', (ar) => ar.student_id === student.id);
...
app.get('/api/reports/class/:classId', (req, res) => {
  ...
  const studentReports = classStudents.map((cs) => {
    const st = await db.findById('students', cs.student_id);
    const prof = st ? await db.findById('profiles', st.profile_id) : null;
...
app.get('/api/reports/course/:courseId', (req, res) => {
  const course = await db.findById('courses', req.params.courseId);
  if (!course) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Course not found.' } });
  }

  const classes = await db.find('classes', (c) => c.course_id === course.id);
  const enrollments = await db.find('enrollments', (e) => e.course_id === course.id);
  const assignments = await db.find('assignments', (a) => a.course_id === course.id);
  const exams = await db.find('examinations', (e) => e.course_id === course.id);
...
app.get('/api/notifications', (req, res) => {
  ...
  const notifications = await db.find('notifications', (n) => n.profile_id === auth.profile.id).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
...
app.put('/api/notifications/read-all', (req, res) => {
  ...
  const notifications = await db.find('notifications', (n) => n.profile_id === auth.profile.id);
...
app.get('/api/audit-logs', (req, res) => {
  ...
  const logs = await db.find('audit_logs').sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 100);
...
app.get('/api/users/students', (req, res) => {
  const students = await db.find('students').map((st) => {
    const profile = await db.findById('profiles', st.profile_id);
    return { ...st, profile };
  });
...
app.get('/api/users/teachers', (req, res) => {
  const teachers = await db.find('teachers').map((t) => {
    const profile = await db.findById('profiles', t.profile_id);
    return { ...t, profile };
  });
...
app.post('/api/users/students', (req, res) => {
  ...
  await db.logAudit('STUDENT_CREATED', 'student', student.id, auth.profile.email, auth.profile.id, { roll_number });
...
app.post('/api/users/teachers', (req, res) => {
  ...
  await db.logAudit('TEACHER_CREATED', 'teacher', teacher.id, auth.profile.email, auth.profile.id, { employee_code });
...
app.put('/api/users/profile', (req, res) => {
  ...
  if (email !== undefined && email.trim().toLowerCase() !== auth.profile.email.toLowerCase()) {
    ...
    const existing = await db.find('profiles', (p) => p.email.toLowerCase() === cleanEmail && p.id !== auth.profile.id)[0];
    if (existing) {
  ...
  const updatedProfile = await db.update('profiles', auth.profile.id, profileUpdates);
  ...
  if (auth.profile.role === 'STUDENT' && auth.student) {
    ...
    updatedStudent = await db.update('students', auth.student.id, studentUpdates);
  }
  ...
  if (auth.profile.role === 'TEACHER' && auth.teacher) {
    ...
    updatedTeacher = await db.update('teachers', auth.teacher.id, teacherUpdates);
  }

  await db.logAudit('PROFILE_UPDATED', 'profile', auth.profile.id, auth.profile.email, auth.profile.id, {
...
app.get('/api/student/my-courses', (req, res) => {
  ...
  const enrollments = await db.find('enrollments', (e) => e.student_id === auth.student!.id && e.status === 'ENROLLED');
  const courseIds = Array.from(new Set(enrollments.map((e) => e.course_id)));
  const courses = courseIds.map((cid) => await db.findById('courses', cid)).filter(Boolean);
  ...
  if (role === 'STUDENT') {
    ...
    if (Array.isArray(support_subject_ids) && support_subject_ids.length > 0) {
      supportSubjectNames = support_subject_ids
        .map((id: string) => {
          const c = await db.findById('courses', id);
    ...
    // Update Profile
    const updatedProfile = await db.update('profiles', auth.profile.id, {
    ...
    if (auth.student) {
      ...
      updatedStudent = await db.update('students', auth.student.id, {
        major: studentPreferences.department,
        semester: derivedSemester
      });
    }

    await db.logAudit('STUDENT_ONBOARDING_COMPLETED', 'profile', auth.profile.id, auth.profile.email, auth.profile.id, {
    ...
    if (auth.teacher) {
      updatedTeacher = await db.update('teachers', auth.teacher.id, {
        specialization: teacherPreferences.department,
        designation: teacherPreferences.designation
      });
    }

    await db.logAudit('TEACHER_ONBOARDING_COMPLETED', 'profile', auth.profile.id, auth.profile.email, auth.profile.id, {
      department: teacherPreferences.department,
      designation: teacherPreferences.designation
    });
    ...
    await db.logAudit('ADMIN_ONBOARDING_COMPLETED', 'profile', auth.profile.id, auth.profile.email, auth.profile.id);
...
app.get('/api/courses/:id/learn', (req, res) => {
  ...
  if (!studentId && (auth.profile.role === 'TEACHER' || auth.profile.role === 'ADMIN')) {
    ...
    const defaultStudent = await db.find('students')[0];
    studentId = previewStudentId || defaultStudent?.id || 'std_alex_id';
  }

  // Ensure enrollment record exists for registered student
  let enrollment = await db.find('enrollments', (e) => e.course_id === courseId && e.student_id === studentId)[0];
  ...
  const modules = await db.find('course_modules', (m) => m.course_id === courseId).sort((a, b) => a.order_index - b.order_index);
  const allLessons = await db.find('course_lessons', (l) => l.course_id === courseId).sort((a, b) => a.order_index - b.order_index);
  const progressRecords = await db.find('lesson_progress', (lp) => lp.course_id === courseId && lp.student_id === studentId);
  ...
  const enrichedModules = modules.map((m) => {
    const modLessons = allLessons.filter((l) => l.module_id === m.id).map((l) => {
      ...
      let assignment = null;
      if (l.assignment_id) {
        assignment = await db.findById('assignments', l.assignment_id);
        if (assignment) {
          const sub = await db.find('assignment_submissions', (as) => as.assignment_id === assignment.id && as.student_id === studentId)[0];
      ...
      let quiz = null;
      if (l.quiz_id) {
        quiz = await db.findById('quizzes', l.quiz_id);
        if (quiz) {
          const attempts = await db.find('quiz_attempts', (qa) => qa.quiz_id === quiz.id && qa.student_id === studentId);
  ...
  // Upcoming deadlines
  const assignments = await db.find('assignments', (a) => a.course_id === courseId);
  ...
  const submissions = await db.find('assignment_submissions', (s) => s.course_id === courseId && s.student_id === studentId && (s.status === 'GRADED' || s.status === 'EVALUATED'));
  const quizAttempts = await db.find('quiz_attempts', (qa) => qa.course_id === courseId && qa.student_id === studentId);
  
  const recentGrades: any[] = [];
  submissions.forEach((s) => {
    const asg = await db.findById('assignments', s.assignment_id);
    recentGrades.push({
  ...
  quizAttempts.forEach((qa) => {
    const qz = await db.findById('quizzes', qa.quiz_id);
    recentGrades.push({
  ...
  submissions.filter((s) => s.feedback).forEach((s) => {
    const teacherProfile = await db.findById('profiles', course.created_by || 'fac_alan_id');
    const asg = await db.findById('assignments', s.assignment_id);
  ...
  if (course.is_nptel) {
    nptelTracking = await db.find('nptel_tracking', (nt) => nt.course_id === courseId && nt.student_id === studentId)[0];
    if (!nptelTracking) {
      nptelTracking = await db.insert('nptel_tracking', {
        course_id: courseId,
        student_id: studentId,
  ...
  const externalTracking = await db.find('external_courses', (ec) => ec.course_id === courseId && ec.student_id === studentId)[0];
...
app.post('/api/courses/:id/lessons/:lessonId/progress', (req, res) => {
  ...
  const course = await db.findById('courses', courseId);
  const lesson = await db.findById('course_lessons', lessonId);
  if (!course || !lesson) {
  ...
  let existing = await db.find('lesson_progress', (lp) => lp.lesson_id === lessonId && lp.student_id === studentId)[0];
  ...
  let updatedProgress;
  if (existing) {
    updatedProgress = await db.update('lesson_progress', existing.id, {
    ...
    updatedProgress = await db.insert('lesson_progress', {
      student_id: studentId,
      course_id: courseId,
...
app.get('/api/courses/:id/quizzes/:quizId', (req, res) => {
  ...
  const quiz = await db.findById('quizzes', quizId);
  if (!quiz) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Quiz not found.' } });
  }

  const studentId = auth.student ? auth.student.id : 'std_alex_id';
  const attempts = await db.find('quiz_attempts', (qa) => qa.quiz_id === quizId && qa.student_id === studentId);
...
app.post('/api/courses/:id/quizzes/:quizId/submit', (req, res) => {
  ...
  const existingAttempts = await db.find('quiz_attempts', (qa) => qa.quiz_id === quizId && qa.student_id === studentId);
  ...
  if (passed && quiz.lesson_id) {
    const lesson = await db.findById('course_lessons', quiz.lesson_id);
    if (lesson) {
      const existingProg = await db.find('lesson_progress', (lp) => lp.lesson_id === lesson.id && lp.student_id === studentId)[0];
      if (existingProg) {
        await db.update('lesson_progress', existingProg.id, {
          status: 'COMPLETED',
          completed_at: now
        });
      } else {
...
app.post('/api/assignments/:id/submit', (req, res) => {
  ...
  const assignment = await db.findById('assignments', assignmentId);
  if (!assignment) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Assignment not found.' } });
  }
  ...
  let existing = await db.find('assignment_submissions', (s) => s.assignment_id === assignmentId && s.student_id === studentId)[0];
  let submission;
  ...
  if (assignment.lesson_id) {
    const prog = await db.find('lesson_progress', (lp) => lp.lesson_id === assignment.lesson_id && lp.student_id === studentId)[0];
    if (prog) {
      await db.update('lesson_progress', prog.id, { status: 'COMPLETED', completed_at: now });
    } else {
      const lesson = await db.findById('course_lessons', assignment.lesson_id);
      await db.insert('lesson_progress', {
        student_id: studentId,
        course_id: assignment.course_id,
        module_id: lesson?.module_id || '',
  ...
  if (assignment.teacher_id) {
    const teacherProfile = await db.findById('profiles', assignment.teacher_id);
    if (teacherProfile) {
      await db.createNotification(
        teacherProfile.id,
...
app.get('/api/courses/:id/nptel', (req, res) => {
  ...
  const tracking = await db.find('nptel_tracking', (nt) => nt.course_id === courseId && nt.student_id === studentId)[0];
...
app.post('/api/courses/:id/nptel', (req, res) => {
  ...
  const now = new Date().toISOString();
  let existing = await db.find('nptel_tracking', (nt) => nt.course_id === courseId && nt.student_id === studentId)[0];
  let record;

  if (existing) {
    record = await db.update('nptel_tracking', existing.id, {
    ...
    record = await db.insert('nptel_tracking', {
      course_id: courseId,
      student_id: studentId,
...
app.get('/api/courses/:id/external', (req, res) => {
  ...
  const courseId = req.params.id;
  const studentId = auth.student ? auth.student.id : 'std_alex_id';
  const record = await db.find('external_courses', (ec) => ec.course_id === courseId && ec.student_id === studentId)[0];
...
app.post('/api/courses/:id/external', (req, res) => {
  ...
  const now = new Date().toISOString();
  let existing = await db.find('external_courses', (ec) => ec.course_id === courseId && ec.student_id === studentId)[0];
  let record;

  if (existing) {
    record = await db.update('external_courses', existing.id, {
    ...
    record = await db.insert('external_courses', {
      course_id: courseId,
      student_id: studentId,
...
app.get('/api/teacher/courses/:id/monitoring', (req, res) => {
  ...
  const enrollments = await db.find('enrollments', (e) => e.course_id === courseId && e.status !== 'DROPPED');
  const assignments = await db.find('assignments', (a) => a.course_id === courseId);
  const quizzes = await db.find('quizzes', (q) => q.course_id === courseId);
  ...
  const studentRows = enrollments.map((en) => {
    ...
    const classes = await db.find('classes', (c) => c.course_id === courseId);
    const classIds = classes.map((c) => c.id);
    const sessions = await db.find('attendance_sessions', (s) => classIds.includes(s.class_id));
    const sessionIds = sessions.map((s) => s.id);
    ...
    const nptelRecord = await db.find('nptel_tracking', (nt) => nt.course_id === courseId && nt.student_id === en.student_id)[0];
  ...
  const pendingSubmissions = await db.find('assignment_submissions', (as) => as.course_id === courseId && as.status === 'SUBMITTED');
...
app.post('/api/teacher/submissions/:id/grade', (req, res) => {
  ...
  const submission = await db.findById('assignment_submissions', submissionId);
  if (!submission) {
    return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
  }
  ...
  const updated = await db.update('assignment_submissions', submissionId, {
    marks_obtained: Number(marks_obtained),
    feedback: feedback || '',
...
app.post('/api/teacher/external-courses/:id/verify', (req, res) => {
  ...
  let externalRecord = await db.findById('external_courses', id);
  if (externalRecord) {
    const updated = await db.update('external_courses', id, {
    ...
    const student = await db.findById('students', externalRecord.student_id);
    if (student) {
      await db.createNotification(
        student.profile_id,
  ...
  let nptelRecord = await db.findById('nptel_tracking', id);
  if (nptelRecord) {
    const updated = await db.update('nptel_tracking', id, {
...
app.post('/api/courses/:id/modules', (req, res) => {
  ...
  const existingModules = await db.find('course_modules', (m) => m.course_id === courseId);
  const newModule = await db.insert('course_modules', {
    course_id: courseId,
    title: title.trim(),
    description: description || '',
...
app.post('/api/courses/:id/lessons', (req, res) => {
  ...
  const existingLessons = await db.find('course_lessons', (l) => l.module_id === module_id);
  const newLesson = await db.insert('course_lessons', {
    course_id: courseId,
    module_id,
    title: title.trim(),
...
app.get('/api/admin/course-analytics', (req, res) => {
  ...
  const allSubmissions = await db.find('assignment_submissions');
  const allQuizzes = await db.find('quiz_attempts');
...
app.post('/api/ai/sc-edu-ai', async (req, res) => {
  ...
  if (!studentId && auth?.profile?.id) {
    const st = await db.find('students', (s) => s.profile_id === auth.profile.id)[0];
...
app.get('/api/support/tickets', (req, res) => {
  ...
  let tickets = await db.find('support_tickets');
  if (auth.profile.role === 'STUDENT') {
...
app.post('/api/support/tickets', (req, res) => {
  ...
  const now = new Date().toISOString();
  const newTicket = await db.insert('support_tickets', {
...
app.post('/api/support/tickets/:id/reply', (req, res) => {
  ...
  const ticket = await db.findById('support_tickets', ticketId);
  if (!ticket) {
...
app.get('/api/feedback/stats', (req, res) => {
  ...
  let feedbacks = await db.find('feedback');
  if (auth.profile.role === 'STUDENT') {
...
app.get('/api/feedback/my', (req, res) => {
  ...
  const feedbacks = await db.find('feedback', (f) => f.profile_id === auth.profile.id || f.user_id === auth.profile.id);
...
app.get('/api/feedback/:id', (req, res) => {
  ...
  const feedback = await db.find('feedback', (f) => f.id === feedbackId || f.feedback_id === feedbackId)[0];
  if (!feedback) {
...
app.post('/api/feedback', (req, res) => {
  ...
  // Notify Admins
  const admins = await db.find('profiles', (p) => p.role === 'ADMIN');
  ...
  // Log Audit
  await db.logAudit('FEEDBACK_SUBMITTED', 'feedback', newFeedback.id, auth.profile.email, auth.profile.id, {
    feedback_id,
    category,
    subject: subject.trim(),
    page_url
...
app.patch('/api/feedback/:id', (req, res) => {
  ...
  const updated = await db.update('feedback', feedback.id, updates);

  await db.logAudit('FEEDBACK_UPDATED', 'feedback', feedback.id, auth.profile.email, auth.profile.id, {
...
app.post('/api/feedback/:id/internal-note', (req, res) => {
  ...
  const updated = await db.update('feedback', feedback.id, {
    internal_notes,
    updated_at: now
  });
...
app.put('/api/students/profile', (req, res) => {
  ...
  const updatedStudent = await db.update('students', auth.student.id, {
...
app.get('/api/inquiries', (req, res) => {
  ...
  if (auth.profile.role === 'ADMIN' || auth.profile.role === 'TEACHER') {
    const allInquiries = await db.find('inquiries');
    return res.json({ success: true, data: allInquiries });
  }

  const ownInquiries = await db.find('inquiries', (i) => i.user_id === auth.profile.id || i.institutional_email.toLowerCase() === auth.profile.email.toLowerCase());
  return res.json({ success: true, data: ownInquiries });
});

app.get('/api/inquiries/:id', (req, res) => {
  const auth = await getAuthUser(req);
  const inquiry = await db.findById('inquiries', req.params.id);
  if (!inquiry) {
